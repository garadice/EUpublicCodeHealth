...
from connectors.github_client import fetch_repo, parse_github
from sqlalchemy import text
from app.db import engine
...
def _upsert_project(conn, p: dict):
    conn.execute(
        text(
            """
...
def _enrich_repository(conn, p: dict):
...
def run():
    with engine.begin() as conn:
        ...
        try:
            projects = fetch_catalog_projects()
            for p in projects:
                _upsert_project(conn, p)
                _enrich_repository(conn, p)
            conn.execute(text("UPDATE pipeline_runs SET status='success', ended_at=NOW() WHERE run_id=:id"), {"id": run_id})
        except Exception as exc:
            conn.execute(
                text("UPDATE pipeline_runs SET status='failed', ended_at=NOW(), error_summary=:e WHERE run_id=:id"),
                {"id": run_id, "e": str(exc)[:4000]},
            )
